...
class OnDemandFunctionOfPath<T> {
   ...
   *
   * <p>The returned value does not depend on the graph's traversal order; the function is applied
   * per path.
   */
  ...
  /**
   * Returns an instance that, on {@link #summon}, traverses the graph breadth-first starting
   * from all roots (nodes with in-degree 0) simultaneously, and assigns one element of
   * {@code values} to each node in that order.
   *
   * <p>Each call to {@link #summon} calls {@link Iterable#iterator()} afresh, so any
   * collection-backed iterable (e.g. a {@link java.util.List}) can be reused across multiple calls.
   * An infinite iterable (where {@code iterator()} creates a fresh counter each time) is also safe.
   *
   * <p>Among multiple roots, their relative order in the traversal follows the graph's node
   * iteration order (insertion order for {@link com.google.common.graph.ImmutableGraph}).
   */
   ...
   *
   * <p>If created via {@link #function}, returns the wrapped function. If created via
   ...
   * as a function (which throws {@link IllegalArgumentException} for unknown
   * paths).
   */
